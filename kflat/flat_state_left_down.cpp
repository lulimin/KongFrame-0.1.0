	...
	const void* pMsg)
{
	...
	if (event == FLAT_EVENT_TIMER)
	{
		...
		if (pState->fTimeCount >= ENTER_HOVER_TIME)
		{
			flat_state_switch(pState, FLAT_STATE_LEFT_HOVER);
			return 1;
		}
	...
	const void* pMsg)
{
	...
	if (event == FLAT_EVENT_TIMER)
	{
		...
		if (pState->fTimeCount >= HOVER_TIME)
		{
			FlatWidget* pTopDialog = pState->pFlatUI->GetTopDialog();

			if (pTopDialog)
			{
				int x = pState->nMouseX;
				int y = pState->nMouseY;
				FlatWidget* pWidget = pTopDialog->GetRegionOf(x, y);
				...
				if (pWidget)
				{
					...
					if (pWidget == pClicked)
					{
						pWidget->OnMouseHover(x, y);
					}
				}
			}

			pState->fTimeCount -= HOVER_TIME;
		}
